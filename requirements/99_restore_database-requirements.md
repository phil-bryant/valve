...
Tests:
- Run restore with globals replay against an environment that already has target roles and verify script continues to database restore.
...
Tests:
- Run restore with mixed globals content and verify unrelated roles (for example `manifold`, `teller`, personal roles) are not replayed.
