...
Tests:
- R001-T01: Force `psql` failure and verify script exits non-zero.
...
- R005-T03: Return empty valve host/port from `1psa` and verify explicit non-zero failure output.
...
Tests:
- R015-T01: Run script from a non-repo working directory and verify schema file still resolves.

R020  Statement: Refuse deploy when schema file is missing.
Design: Validate `storage/schema.sql` exists before invoking `psql`.
Tests:
- R020-T01: Remove schema file in a fixture and verify explicit non-zero failure output.
