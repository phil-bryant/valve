...
setup_fixture() {
  create_repo_fixture
  copy_script_to_fixture "09_launch_ui.sh"
...
@test "runs from non-repo cwd and resolves UI package from script root" {
  ...
  run env PATH="${PATH}" bash -c "cd '${TEST_TMPDIR}/elsewhere' && bash '${FIXTURE_ROOT}/09_launch_ui.sh'"
  [ "$status" -eq 0 ]
...
@test "fails fast with installer guidance when swift is missing" {
  #R005
  make_swift_unavailable_stub
  run env PATH="${PATH}" bash "${FIXTURE_ROOT}/09_launch_ui.sh"
  [ "$status" -ne 0 ]
...
@test "fails when UI package directory is missing" {
  #R010
  run env PATH="${PATH}" UI_PACKAGE_DIR="${FIXTURE_ROOT}/missing-ui" bash "${FIXTURE_ROOT}/09_launch_ui.sh"
  [ "$status" -ne 0 ]
...
@test "fails when UI package manifest is missing" {
  ...
  run env PATH="${PATH}" UI_PACKAGE_DIR="${FIXTURE_ROOT}/macos/EmptyUI" bash "${FIXTURE_ROOT}/09_launch_ui.sh"
  [ "$status" -ne 0 ]
...
@test "uses swift build for build mode" {
  #R015
  run env PATH="${PATH}" UI_LAUNCH_MODE=build bash "${FIXTURE_ROOT}/09_launch_ui.sh"
  [ "$status" -eq 0 ]
...
@test "fails for unsupported launch mode" {
  #R015
  run env PATH="${PATH}" UI_LAUNCH_MODE=invalid bash "${FIXTURE_ROOT}/09_launch_ui.sh"
  [ "$status" -ne 0 ]
